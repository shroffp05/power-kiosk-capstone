...
def get_client_metrics(df: pd.DataFrame) -> list:
    ...
    client_df = df.loc[(df['period_clean']>=start_date) & (df['period_clean']<=end_date), :]

    print("Start Date: {}".format(start_date))
    print("End Date: {}".format(end_date))

    end_date_2 = df['period_clean'].max().to_pydatetime() -  dateutil.relativedelta.relativedelta(months=12)
    start_date_2 = end_date_2 - dateutil.relativedelta.relativedelta(months=12)

    client_df_2 = df.loc[(df['period_clean']>=start_date_2) & (df['period_clean']<=end_date_2), :]

    print("Start Date: {}".format(start_date_2))
    print("End Date: {}".format(end_date_2))

    return [client_df["clean_usage"].sum()/12, client_df_2["clean_usage"].sum()/12] # Assuming we have atleast 24 months data
